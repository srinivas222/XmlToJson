...
public class CustomerRouter extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		Map<String, String> reference = new HashMap<String, String>();
	        ...
	        /*Map<String, String> reference = new HashMap<String, String>();
	        reference.put("customer", Customer.class.getName());
	        reference.put("customers", Customers.class.getName());
	        
	        XStreamDataFormat xstreamDataFormat = new XStreamDataFormat();
	        xstreamDataFormat.setAliases(reference);
	        
	        xstreamDataFormat.setPermissions(Customers.class.getName());*/

		// ----------------Add the Customer----------------

		rest("/customerPost").post().type(Customer.class).to("direct:customerPost");
		from("direct:customerPost").log("Customer id is ${body}").unmarshal(xstreamDataFormat).log("After unmarshalling ${body}").process(new InsertProcessor())
		...
		/*rest("/MultiCustomerPost").post().type(Customer.class).to("direct:customersPost");
		from("direct:customersPost").log("Customer id is ${body}").unmarshal(xstreamDataFormat).log("After unmarshalling ${body}").process(new InsertProcessor())
				.log("the message in the headers is:${headers.id},${headers.name},${headers.email},${headers.location}")
				//.setBody(simple(
					//	"insert into customer (cid,cname,email,location) values(${headers.id},'${headers.name}','${headers.email}','${headers.location}')"))
				//.to("jdbc:datasource")
